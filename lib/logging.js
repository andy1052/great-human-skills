...
console.log("requestId from logging.js: ", requestId);
...
logging.list = function(includeCompressedLogs, callback) {
	...
	fs.readdir(logging.baseDir, (err, data) => {
		...
		if (!err && data && data.length >0) {
			...
			console.log("There's been an error in logging.list!");
			return err;
		};
	});
};
...
logging.compress = function(logId, newField, callback, next) {
...
console.log("destFile: ", logging.baseDir + destFile);
...
logging. decompress = function(fileId, callback, next) {
...
logging.truncate = function(logId, callback, next) {
