...
helpers2.resetPassEmail = async function(data) {
	...
	console.log("HELPERS2 DATA: ", data);
	...
	let token = typeof(user.token) === 'string' && user.token.trim().length > 0 && user.token.trim().length < 20 ? user.token.trim() : false;

	try {

		//************ YOU NEED TO FIGURE OUT THE URL PART SO THAT IT WORKS, OTHERWISE, IT'S GREAT! *************************88
		...
		<body>
			<div>
				<h3>Dear ${username},</h3>
				<p>You requested a password reset for your Great Human Skills account. Please use <a href="http://localhost:3000/resetToken/${token}">this link</a> to do so.</p>
				<br>
				<p>If you did not request this password reset, please let us know <a href="/">here</a></p>
				<p>Thanks for your time and we hope to see you soon at <a href="/">Great Human Skills</a></p>
			</div>
